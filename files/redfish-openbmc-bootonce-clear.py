...
if override_mode == "UEFI":
    if override_target != "None" or override_enabled != "Disabled" :
    
        url      = 'https://%s/redfish/v1/Systems/1' % bmc_ip
        headers  = {'content-type': 'application/json', 'If-Match': override_etag}
        payload  = {'Boot':{'BootSourceOverrideEnabled': 'Disabled','BootSourceOverrideTarget': 'None'}}
        ...
        print("SUCCESS: no bootonce setting detected/enabled")
else:
