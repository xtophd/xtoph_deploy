...
if vmedia_state == True:
    ...
    url      = 'https://%s/redfish/v1/Managers/1/VirtualMedia/CD/Actions/VirtualMedia.EjectMedia' % bmc_ip
    headers  = {'content-type': 'application/json'}

    print("Attempting Ejection", file=sys.stderr)

    response = requests.post(url, headers=headers, auth=(bmc_username, bmc_password), verify=False)

    result_code = response.status_code

    if result_code == 200:
        print("SUCCESS: vmedia eject result code %s" % result_code)
        sys.exit(0)
    else:
        print("FAIL: vmedia eject result code %s" % result_code)
        print(response)
        sys.exit(1)



print("SUCCESS: vmedia eject unnecessary")
