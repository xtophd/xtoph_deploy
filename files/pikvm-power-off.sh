...
##    power state of a host to 'off' using pi-kvm API
##
...
##
##    curl --insecure -s -u "$bmc_uid:$bmc_pw" https://$bmc_ip/api/atx/power?action=off_hard
##
...
RESPONSE=$( curl --insecure -s -k -X POST -u $bmc_username:$bmc_password https://$bmc_ip/api/atx/click?button=power_long | jq -r '.ok' | tr [:upper:] [:lower:] )
